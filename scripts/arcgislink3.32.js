...
/* V3.32 CHRISTINA, TRY TO FORCE LOAD ONCE */
var loadOnce = true;
...
function addCopyrightInfo_(cpArray, mapService, map) {
  /*if (mapService.hasLoaded()) {
	// removed to prevent copyright text covering Center Map button
...
/* V3.32 CHRISTINA added to compare params, return null on exportmap if same */
    this._lastparams;
...
function MapOverlay(service, opt_overlayOpts, id) {
  this._id = id;  // for [LOG]
      ...
      /* [QUESTION] How early should _drawing be set to false? Should it do anything? - CLarson 8/13/18 */
		...
		if (json.href){
			...
			if( !(returnVal_current.boundsChanged) || !(returnVal_current.sizeChanged) ){
				...
				//me.overlay_ = new ImageOverlay(json.bounds, json.href, me.map_, me.opacity_,me.__ZINDEX, me._id);
				self.overlay_ = new ImageOverlay(fSizeChanged, fWidth, fHeight, fBounds, fBoundsSW, fBoundsNE, json.href, self.map_, self.opacity_, self.__ZINDEX, self._id, fCalledFrom);
