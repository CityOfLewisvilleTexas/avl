...
/* V3.32 Function FULLY Commented out - clarson */
function addCopyrightInfo_(cpArray, mapService, map) {
  /*if (mapService.hasLoaded()) {
	// Removed to prevent copyright text covering Center Map button - clarson
 ...
 * @property {Array.string} [services] list of services. Each has <code>name, type</code> property.
  ...
  function Catalog(url) {
    ...
    getJSON_(url, {}, '', function(json) {
       ...
       * @event
       */
//	console.log('TRIGGER Catalog load')
...
Layer.prototype.load = function() {
	...
	console.group("Layer.prototype.load")
...
//	console.log('Layer query');
	...
	console.group("arcgislink Layer.prototype.query")
	console.groupEnd();
...
//	console.log('Layer - queryRelatedRecords');
    if (!qparams) {
      return;
    } 
...
/* V3.32 - Added to compare params, return null on exportmap if same - clarson */
    this._lastparams;
    this._countExportCalled = 0;
    this._countExported = 0;

	/* jkirkland's updates copied from maps - clarson */
	this.__TOKEN = (opts.__TOKEN ? opts.__TOKEN : "");  //Added by jkirkland 6/12/2017
    this.__LAYERSETTINGS = opts.__LAYERSETTINGS;  //Added by jkirkland 6/12/2017
	...
	//----------------------------------------------------------
	  //Allow specific layers	(added by JKIRKLAN 1/24/17)  
...
//console.log('MapService - loadServiceInfo');
    var me = this;
	/* changed to match jkirkland's updates copied from maps - clarson */
    //getJSON_(this.url, {}, '', function(json) {
    //me.init_(json);
	var _url = me.url;
    if (me.url.indexOf("?") == -1 && me.url.indexOf("token=") == -1 && me.__TOKEN.length > 0){
		_url = _url + "?token=" + me.__TOKEN; //Added by jkirkland 6/12/2017
    }
    getJSON_(_url, {}, '', function(json) {
		me.init_(json);
    });
  };
  /**
  ...
  MapService.prototype.init_ = function (json) {
	//var _me = this;
	//console.log("MapService.init_"  + _me.__LAYERSETTINGS.label)
    var me = this;
    if (json.error) {
...
//console.log('MapService - load');
  ...
  MapService.prototype.initLayers_ = function (json2) {
	//var _me = this;
	//console.log("MapService.initLayers_"  + _me.__LAYERSETTINGS.label)
    var layers = [];
    var tables = [];
    this.layers = layers;
	...
	//console.group("MapService.prototype.initLayers_")
	//console.log("layers = " + json2.layers.length)	  
	//console.groupEnd()
    ...
    for (i = 0, c = json2.layers.length; i < c; i++) {
	  ...
	  //----------------------------------------------------------
	  //Allow specific layers	(added by JKIRKLAN 1/24/17)  
	/* Removed to match jkirkland's version on maps - clarson */
	/*
	  if (this.layerids.length > 0){
		  if (this.layerids.indexOf(layer.name) > -1){
			  layer.visible = true;
			  layers.push(layer);
		  }
	  }else{
...
//	console.log("MapService.exportMap : " + _me.__LAYERSETTINGS.label)
//	console.log("--" + JSON.stringify(p))
    this._countExportCalled++;
//  console.log("exportMap " + this._countExportCalled + ': ' + this.url.replace('https://adaptergis.cityoflewisville.com/arcgis/rest/services/', ''));
    if (!p || !p.bounds) {
      return;
    }
//    console.log(p.f);
    ...
    params.f = p.f;
    var bnds = p.bounds;
	/* Tested adding sw, nw, sw_lng, and ne_lng to p, but is unnecessary - clarson */
	//if(!(p.boundsSW) || !(p.boundsNE)){
	...
	//}
	/*else{
		var swx = p.boundsSW_lng;
		var nex = p.boundsNE_lng;
		if (swx> nex){
		  swx = swx-180;
	  //    nex = nex +180;
		}
		params.bbox = '' + swx + ',' + '' + p.boundsSW.lat() + ',' +
		nex +
		',' +
		'' +
		p.boundsNE.lat();
	...
	/* Added check to see prevent requesting the same image from arcgis multiple times in a row, but is unnecessary if change in bounds is correctly calculated before - clarson */
//    console.log(formatParams_(params));
    if(this._lastparams == formatParams_(params)){
//      console.log("******************");
      //console.log("******************");
    ...
    this._lastparams = formatParams_(params);
    this._countExported++;

//    console.log("Map exported " + this._countExported + ': ' + this.url.replace('https://adaptergis.cityoflewisville.com/arcgis/rest/services/', ''));
...
//          console.log('errback?');
    ...
    /*Added by jkirkland 12/13/2016 to handle /ImageServer format*/ 
	/* Copied to match jkirkland's version from maps */
	//Build url and add-in token
	var _url = "";
	_url = this.url + "/exportImage";
	if (_url.indexOf("?") == -1 && _url.indexOf("token=") == -1 && this.__TOKEN.length > 0){
		_url = _url + "?token=" + this.__TOKEN; //Added by jkirkland 6/12/2017
	}
	//console.log("--url = " + _url)
  if (params.f === 'image' && params) {
 //   console.log('image & params');
		/* Modified to match jkirklands from maps - clarson */
      //return this.url + '/exportImage?' + formatParams_(params);
		return _url + '?' + formatParams_(params);
    } else {
//      console.log('not image & params');
		/* Modified to match jkirklands from maps - clarson */
      //getJSON_(this.url + '/exportImage', params, '', function (json) {
		getJSON_(_url, params, '', function (json){
//        console.log(json);
        if (json.extent) {
          json.bounds = fromEnvelopeToLatLngBounds_(json.extent);
          //log_('got '+json.bounds.toUrlValue());
 ...
 */
  ...
  GeocodeService.prototype.init_ = function (json) {
     ...
     * @event
     */
//	console.log('GeocodeService - load');
    triggerEvent_(this, 'load');
  };
...
//	console.log('GPService - load');
  ...
  function GPTask(url) {
     ...
     * @event
     */
//	console.log('TRIGGER GPTask load');
      triggerEvent_(me, 'load');
    });
  }
  /**
  ...
  getJSON_(url, {}, '', function(json) {
     ...
     * @event
     */
//	console.log('TRIGGER NetworkService load');
      triggerEvent_(me, 'load');
    });
  }
...
//	console.log('MAPTYPE setOpacity');
    this.opacity_ = op;
    var tiles = this.tiles_;
    for (var x in tiles) {
      if (tiles.hasOwnProperty(x)) {
...
//	console.log('MAPTYPE getOpacity');
    return this.opacity_;
  };
  /**
...
//	console.log('MAPTYPE getTileLayers');
   ...
   */
/* V3.32 CHRISTINA ADDED PARAMETER FOR ID */
function MapOverlay(service, opt_overlayOpts) {
	/* [DESC] - CLarson 8/13/18 - Google Maps V3.32
	- Draw iteration variables for checking
	- Set only for specific instances when draw is called (because draw is called so often)
	- [USE] Make sure a full draw call is able to set the variables needed before another draw is called
	----- */
	this._checking = false;   /* to make sure vars are set for this draw before another draw starts changing vars */
	this._checkingBounds = false;
	this._checkingDraw = false;
	this._checkDrawAtCount = 0;
	this._loadNew = true;   /* [REMOVE?] This might be to make sure it draws the first time before map is loaded fully [QUESTION] Map loading may have been done incorrectly */
	this._drawing = false;    /* if currently drawing??? */  /* [ORIGINAL] - There was an original drawing, may have been set in different places */
	this._drawCount = 0;    /* for [LOG] and to set drawAtCount, to only do everything once for a specific draw call */
	this._refreshCount = 0;   /* for [LOG] */
	this._boundsChangedCount = 0;
	this._idleCount = 0;
	this._dragEndCount = 0;

	this._dragging = false;
	this._needsNewImg = false;  /* make sure a new image will be fetched after a drag ends */ 
	/* CLarson 8/13/18
	- [QUESTION] Do we need another refresh. Normally happens bounds changed before server returns image. Might be same as _loadNew */
	this.needsNewRefresh_ = true; /* [ORIGINAL] */      //Changed to true by JKIRKLAN on 1/25/17 to make sure layers with layerids are refreshed when first created so they don't initially show all layers
	/* ----- */

	/* [DESC] - CLarson 8/13/18 - Google Maps V3.32
	- Current variables
	- Set every time draw is called (very often)
	- [USE] For comparing - make sure every overlay only refreshes if changed
	----- */
	this._currentWidth;
	this._currentHeight;
	this._currentBounds;
	this._currentBoundsSW;
	this._currentBoundsNE;
	this._currentBoundsSW_lng;
	this._currentBoundsNE_lng;
	/* ----- */

	/* [DESC] - CLarson 8/13/18 - Google Maps V3.32
	- Last variables
	- Set every time refresh will be (is?) called
	- [USE] For comparing - make sure every overlay only refreshes if changed
	----- */
	this._lastWidth;
	this._lastHeight;
	this._lastBounds;
	this._lastBoundsSW;   /* [REMOVE?] probably don't need */
	this._lastBoundsNE;   /* [REMOVE?] probably don't need */
	this._lastBoundsSW_lng;
	this._lastBoundsNE_lng;
	/* ----- */

	/* [ORIGINAL arcgislink vars] - CLarson 8/13/18
	----- */

	this.mapService_ = (service instanceof MapService) ? service : new MapService(service);

	/* CLarson 8/13/18
	- [QUESTION] Why isn't it a this var? Should this be commented out and add this var instead? Tried, probably not
	I think this is because the actual this needed is split out into minZoom, maxZoom, export_options, etc
	//  this.overlayOpts_ = opt_overlayOpts || {}; */
	...
	/* [JKirkland vars] - CLarson 8/13/18 */
	this.__ZINDEX = opt_overlayOpts.__ZINDEX || 0;
	this.__TOKEN = opt_overlayOpts.__TOKEN || null;  //Added by jkirkland 6/12/2017 /* copied 8/17/18 - clarson */
	...
	this.params = opt_overlayOpts.exportOptions || {};  /* probably don't need, duplicate, except .imagesr is only set on params - CLarson 8/13/18 - Google Maps V3.32 */
	this._id = opt_overlayOpts.__id;

	// Once the LatLng and text are set, add the overlay to the map.  This will
	// trigger a call to panes_changed which should in turn call draw.
	/* CLarson 8/13/18
	- Set this.map_ will be set onAdd
	- [QUESTION] I don't know if you could or should set in create.
	.agsOverlays is added to map onAdd though, and this is pushed onto it */
	if (opt_overlayOpts.map) {
		//console.log('TRIGGER opt_overlayOpts');
		this.setMap(opt_overlayOpts.map);
	}
	this.map_ = null;   /* this.getMap(), this isn't a copy, it is the actual Map node (no need to pass around) - CLarson 8/13/18 */
	this.listeners_= [];  /* will use this to remove listeners onRemove - CLarson 8/13/18 */

    /* ----- */

  /* CLarson 8/13/18 - Google Maps V3.32
  - [REMOVE?] Not sure why added both vars anymore */
    this.overlay_ = null;
    this.div_ = null;
};
   ...
   */
  /* CLarson 8/~/18
- This will be called after setMap(map) is called, as long as map is not null */
MapOverlay.prototype.onAdd = function() {

    var self = this;

  /* [LOG] - CLarson 8/~/18 */
  //console.log(this._id + " MO.onAdd - ");


  /* [DESC] - CLarson 8/~/18
  - Add Listeners
  ----- */
    this.listeners_.push(G.event.addListener(this.getMap(), 'g-bounds_changed', function(){
    /* [LOG] - CLarson 8/~/18 */
    console.log('Event: G-Bounds_Changed');
    }));
	this.listeners_.push(G.event.addListener(this.getMap(), 'idle', function(){
		/* [LOG] - CLarson 8/13/18 */
	    //console.log('Event: IDLE - ' + self._id);

		/* CLarson 8/13/18 - Google Maps V3.32
		- [DESC] Current function variables
		----- */
		var _fMap = self.getMap();

		if(!_fMap){
			//console.log('Map not loaded - ' + self._id + ' [idle]');
			return;
		}
		...
		var fCount = ++self._idleCount;
		var fCalledFrom =  ' [idle ' + fCount + ']'

		var _fDiv = _fMap ? _fMap.getDiv() : null;
		var _fBounds = _fMap ? _fMap.getBounds() : null;
		var _fWidth, _fHeight, _fBoundsSW, _fBoundsNE, _fBoundsSW_lng, _fBoundsNE_lng;
		/* ----- */

		if(self._checkingBounds){
			//console.log('*** ERRORR??? *** Still checking bounds [bounds_changed]');
		}
		if(self._checkingDraw){
			//console.log('*** ERRORR??? *** Still checking draw [bounds_changed]');
		}
		/* CLarson 8/13/18 - Google Maps V3.32
		- Set function variables
		- Set self. current variables as function variables
		----- */
		self._currentWidth = _fWidth = _fDiv ? _fDiv.offsetWidth : null;
		self._currentHeight = _fHeight = _fDiv ? _fDiv.offsetHeight : null;
		self._currentBounds = _fBounds;
		self._currentBoundsSW = _fBoundsSW = _fBounds ? _fBounds.getSouthWest() : null;
		self._currentBoundsNE = _fBoundsNE = _fBounds ? _fBounds.getNorthEast() : null;
		self._currentBoundsSW_lng = _fBoundsSW_lng = _fBoundsSW ? _fBoundsSW.lng() : null;
		self._currentBoundsNE_lng = _fBoundsNE_lng = _fBoundsNE ? _fBoundsNE.lng() : null;
		/* ----- */
		
		//console.log('bounds_changed > ' + _fBoundsSW_lng + ', ' + _fBoundsNE_lng);

		self.checkDraw(_fWidth, _fHeight, _fBounds, _fBoundsSW, _fBoundsNE, _fBoundsSW_lng, _fBoundsNE_lng, fCalledFrom);
    }));
  /* CLarson 8/~/18
  - [FYI] bound_changed was commented out in original arcgislink - nothing extra done for it, draw is always called on it's own
  - [QUESTION] Draw might not be called or called correctly when centerV changed with Vue (on centerCallMarker etc) */
  //this.listeners_.push(G.event.addListener(this.getMap(), 'bounds_changed', callback_(this.refresh, this))); /*[ORIGINAL]
  this.listeners_.push(G.event.addListener(this.getMap(), 'bounds_changed', function(){
    /* [LOG] - CLarson 8/13/18 */
	    //console.log('Event: Bounds Changed - ' + self._id);
		...
		if(!_fMap){
			//console.log('Map not loaded - ' + self._id + ' [bounds_changed]');
			return;
		}
		...
		var fCount = ++self._boundsChangedCount;
		var fCalledFrom =  ' [bounds_changed ' + fCount + ']'
    ...
    this.listeners_.push(G.event.addListener(this.getMap(), 'dragend', function(){
		...
		self._needsNewImg = true;	/* V3.32 CLARSON 8/13/18 - Make sure a new image is fetched at the end of dragend */

		/*var _fMap = self.getMap();

		if(!_fMap){
			//console.log('Map not loaded - ' + self._id + ' [dragend]');
			return;
		}

		var fCount = ++self._dragEndCount;
		var fCalledFrom = ' [dragend ' + fCount + ']'
		...
		if(self._checkingBounds){
			//console.log('*** ERRORR??? *** Still checking bounds ' + self._id + ' [dragend]');
		}
		if(self._checkingDraw){
			//console.log('*** ERRORR??? *** Still checking draw ' + self._id + ' [dragend]');
		}
  ...
  /* CLarson ~/~/18 (Changed when custom Center Map button was added to the AVL)
  - Commented out setCopyright because it can cover Center Map button making it unable to click
  - Copyright is Tarig's COL, not Google's */
  /* [QUESTION] CLarson 8/13/18
    - maybe could keep this if the zIndex is set lower for the copyright? */
    //setCopyrightInfo_(map);

    this.map_ = map;

  /* CLarson 8/~/18 - Google Maps V3.32
  - Moved from draw or refresh because sr will only need to be set once 
  - [FYI] this will be the same for every layer of ours as of 8/13/18 (WEB_MERCATOR) */
  var sr = WEB_MERCATOR;
    var prj = map.getProjection(); // note this is not same as this.getProjection which returns MapCanvasProjection
    if (prj && prj instanceof Projection) {
      sr = prj.spatialReference_;
    }
    this.params.imageSR = sr;

  /* CLarson 8/13/18
  - not sure why this is here or why commented out */
    //this.overlay_ = new ImageOverlay(this.map_, this.opacity_,this.__ZINDEX, this._id);
	//console.log("onAdd");
	//this.checkDraw(this._currentWidth, this._currentHeight, this._currentBounds, this._currentBoundsSW, this._currentBoundsNE, this._currentBoundsSW_lng, this._currentBoundsNE_lng, ' [onAdd]');
};
...
/* V3.32 CHRISTINA ADDED CONSOLE */
    //console.log(this._id + " MO.onRemove");
...
/* V3.32 CHRISTINA NOTE: Remove overlay_ - call ImageOverlay.onRemove */
    if (this.overlay_){
...
/* V3.32 CHRISTINA COMMENTED OUT */
   ...
   */
     /* CLarson 8/~/18
- This will be very often by Google API, no longer have to make sure it's called
- [FYI] keep function variables to call refresh with. Don't use this. variables because multiple draw calls may interfere
- [FYI] Don't add a console.log to the draw before any of the draw iteration variables are set UNLESS only one layer is being drawn - there's too many, so it's not useful.
- [PROBLEM 8/13/18] The overlay image is removed but not added again when centerV is set on centerCallMarker [specifically for fire] */
MapOverlay.prototype.draw = function () {
	...
	if(this._loadNew){
		...
		- [DESC] Current function variables
		----- */
		var _fMap = this.getMap();

		if(!_fMap){
			//console.log('Map not loaded - ' + this._id + ' [first draw]');
			return;
		}

		var fCalledFrom =  ' [first draw]';
		...
		if(this._checkingBounds){
			//console.log('*** ERRORR??? *** Still checking bounds [first draw]');
		}
		if(this._checkingDraw){
			//console.log('*** ERRORR??? *** Still checking draw [first draw]');
		}
		...
		- Set self. current variables as function variables
		----- */
		this._currentWidth = _fWidth = _fDiv ? _fDiv.offsetWidth : null;
		this._currentHeight = _fHeight = _fDiv ? _fDiv.offsetHeight : null;
		this._currentBounds = _fBounds;
		this._currentBoundsSW = _fBoundsSW = _fBounds ? _fBounds.getSouthWest() : null;
		this._currentBoundsNE = _fBoundsNE = _fBounds ? _fBounds.getNorthEast() : null;
		this._currentBoundsSW_lng = _fBoundsSW_lng = _fBoundsSW ? _fBoundsSW.lng() : null;
		this._currentBoundsNE_lng = _fBoundsNE_lng = _fBoundsNE ? _fBoundsNE.lng() : null;
		/* ----- */
		
		this.checkDraw(_fWidth, _fHeight, _fBounds, _fBoundsSW, _fBoundsNE, _fBoundsSW_lng, _fBoundsNE_lng, fCalledFrom);
	}
};

MapOverlay.prototype.checkDraw = function (fWidth, fHeight, fBounds, fBoundsSW, fBoundsNE, fBoundsSW_lng, fBoundsNE_lng, calledFrom) {
	/* [Log] CLarson 8/13/18 */
	//console.log(this._id + " MO.checkDraw");

	this._checkingDraw = true;
	var dragEnd = false;
	var i = calledFrom.search('dragend');
	if(i > 0){
		dragEnd = true;
	}
	
	// Check current first
	// May have to try something like using dragEnd var instead of default false
	var returnVal_current = this.checkBounds(fWidth, fHeight, fBoundsSW_lng, fBoundsNE_lng, 'current', false);
	if(!returnVal_current.compared){
		if(calledFrom == ' [onAdd]'){
			if(!fWidth || !fHeight){
				//console.log('  //' + calledFrom + ' check current: no width/height - ' + returnVal_current.msg);
			}
			if(!fBoundsSW_lng || !fBoundsNE_lng){
				//console.log('  //' + calledFrom + ' check current: no bounds - ' + returnVal_current.msg);
			}
		}
		else{
			//console.log('  //' + calledFrom + ' check current: no compare - ' + returnVal_current.msg);
		}
		return;
	}
	else{
		//console.log('  //' + calledFrom + ' check current - ' + returnVal_current.msg);
	}

	if(returnVal_current.needsDraw){
		//console.log('DRAW CALL REFRESH - CURRENT ' + fBoundsSW_lng + ', ' + fBoundsNE_lng + calledFrom);

		this._lastWidth = fWidth;
		this._lastHeight = fHeight;
		this._lastBounds = fBounds;    //UNNEEEDED
		this._lastBoundsSW = fBoundsSW;  /* THIS WAS PROBABLY THE PROBLEM FRIDAY - set to _lng - CLarson 8/13/18*/ //UNNEEEDED
		this._lastBoundsNE = fBoundsNE;  /* THIS WAS PROBABLY THE PROBLEM FRIDAY - set to _lng - CLarson 8/13/18*/ //UNNEEEDED
		this._lastBoundsSW_lng = fBoundsSW_lng;
		this._lastBoundsNE_lng = fBoundsNE_lng;

		this._checkingDraw = false;
		if(returnVal_current.endDragging){
			//this._dragging = false;
		}
		this.refresh(fWidth, fHeight, fBounds, fBoundsSW, fBoundsNE, fBoundsSW_lng, fBoundsNE_lng, returnVal_current.boundsChanged, returnVal_current.sizeChanged, returnVal_current.needsNewImg, calledFrom);
		return;
	}

	// Check last next
	var returnVal_last = this.checkBounds(fWidth, fHeight, fBoundsSW_lng, fBoundsNE_lng, 'last', false);
	if(!returnVal_last.compared){
		if(calledFrom == ' [onAdd]'){
			if(!fWidth || !fHeight){
				//console.log('  //' + calledFrom + ' check last: no width/height - ' + returnVal_current.msg);
			}
			if(!fBoundsSW_lng || !fBoundsNE_lng){
				//console.log('  //' + calledFrom + ' check last: no bounds - ' + returnVal_current.msg);
			}
		}
		else{
			//console.log('  //' + calledFrom + ' check last: no compare - ' + returnVal_current.msg);
		}
	}
	else{
		//console.log('  //' + calledFrom + ' check last - ' + returnVal_last.msg);
	}

	if(returnVal_last.needsDraw){
		//(fWidth, fHeight, fBoundsSW, fBoundsNE, fBoundsSW_lng, fBoundsNE_lng, returnVal_last.boundsChanged, returnVal_last.sizeChanged, returnVal_last.needsNewImg)
		//console.log('DRAW CALL REFRESH - LAST ' + fBoundsSW_lng + ', ' + fBoundsNE_lng + calledFrom);
		...
		this._checkingDraw = false;
		if(returnVal_last.endDragging){
			//this._dragging = false;
		}
		this.refresh(fWidth, fHeight, fBounds, fBoundsSW, fBoundsNE, fBoundsSW_lng, fBoundsNE_lng, returnVal_last.boundsChanged, returnVal_last.sizeChanged, returnVal_last.needsNewImg, calledFrom);
		return;
	}
	
	this._checkingDraw = false;
	if(returnVal_current.endDragging || returnVal_last.endDragging){
		//this._dragging = false;
	}
	return;
};

/* returns null, true, or fall if bounds provided are different than current or last */
MapOverlay.prototype.checkBounds = function (fWidth, fHeight, fBoundsSW_lng, fBoundsNE_lng, checkAgainst, dragEnd) {
	var returnVal = {
		needsDraw: false,
		msg: '',
		compared: false,
	...
	/* [Log] CLarson 8/13/18 */
	//console.log(this._id + " MO.checkBounds " + checkAgainst);
	/* Current function variables - CLarson 8/13/18 */
	var sameBoundsSW_lng, sameBoundsNE_lng, sameBounds, sameWidth, sameHeight, sameSize;

	/* CLarson 8/13/18 - Google Maps V3.32
	- If MapOverlay is not currently in a draw, make sure this draw call will compare the last val to the current val
	- Added check if not dragging so don't have to reset any vals before returning */
	/*if(!this._checkingBounds){*/
	if( !(this.getMap()) ){
		/* [LOG] - CLarson 8/~/18 */
		//console.log('- SKIP check: No map - ' + this._id);
		returnVal.msg = 'SKIP - No map';
		return returnVal;
	}
	if(this._dragging){ //&& !dragEnd){
		/* [LOG] - CLarson 8/~/18 */
		//console.log('- SKIP check: Dragging - ' + this._id);
		returnVal.msg = 'SKIP - Dragging';
		return returnVal;
	}
	/*else if (this._drawing){
		/* [LOG] - CLarson 8/~/18 *
	console.log('- END check: drawing - ' + this._id);
	return false;
	}*/

	this._checkingBounds = true;
  
    /* CLarson 8/13/18
    - if no no bounds, no div, or current width and height are 0, do nothing */
    if (!fBoundsSW_lng || !fBoundsNE_lng || !fWidth || !fHeight){
	    /* [LOG] CLarson 8/13/18 */
	    //console.log('- END check: Missing param - ' + this._id);
	    returnVal.msg = 'END - Missing param';
	    this._checkingBounds = false;
	    return returnVal;
    }
    if(fWidth == 0 && fHeight == 0){
    	/* [LOG] CLarson 8/13/18 */
	    //console.log('- END Check: No width or height sent - ' + this._id);
	    returnVal.msg = 'END - No width or height sent';
	    this._checkingBounds = false;
	    return returnVal;
    }

    // reached compare point
    returnVal.compared = true;

    // check against current vars
    if(checkAgainst == 'current'){
    	 /* bounds are the same - checking _lng - CLarson 8/13/18 */
		sameBoundsSW_lng = ( (!fBoundsSW_lng && !this._currentBoundsSW_lng) || fBoundsSW_lng == this._currentBoundsSW_lng );
		sameBoundsNE_lng = ( (!fBoundsNE_lng && !this._currentBoundsNE_lng) || fBoundsNE_lng == this._currentBoundsNE_lng );
		sameBounds = (sameBoundsSW_lng && sameBoundsNE_lng);
		
		returnVal.boundsChanged = (!sameBounds);

		sameWidth = ( (!fWidth && !this._currentWidth) || fWidth == this._currentWidth);
		sameHeight = ( (!fHeight && !this._currentHeight) || fHeight == this._currentHeight);
		sameSize = (sameWidth && sameHeight);

		returnVal.sizeChanged = (!sameSize);
    }
    // check against last vars
    else if(checkAgainst == 'last'){
    	 /* bounds are the same - checking _lng - CLarson 8/13/18 */
		sameBoundsSW_lng = ( (!fBoundsSW_lng && !this._lastBoundsSW_lng) || fBoundsSW_lng == this._lastBoundsSW_lng );
		sameBoundsNE_lng = ( (!fBoundsNE_lng && !this._lastBoundsNE_lng) || fBoundsNE_lng == this._lastBoundsNE_lng );
		sameBounds = (sameBoundsSW_lng && sameBoundsNE_lng);
		
		returnVal.boundsChanged = (!sameBounds);

		sameWidth = ( (!fWidth && !this._lastWidth) || fWidth == this._lastWidth);
		sameHeight = ( (!fHeight && !this._lastHeight) || fHeight == this._lastHeight);
		sameSize = (sameWidth && sameHeight);

		returnVal.sizeChanged = (!sameSize);
    }

    // check if needsNewImg
	if (this._needsNewImg){
		returnVal.needsNewImg = true;
		this._needsNewImg = false; // can this be set here?
	}

	if(returnVal.boundsChanged || returnVal.sizeChanged || returnVal.needsNewImg){
		returnVal.needsDraw = true;
	}

	this._checkingBounds = false;

	//console.log('- END Check: Valued Compared - ' + this._id);
	returnVal.msg = 'END - needsDraw: ' + returnVal.boundsChanged.toString() + ' ((boundsChanged: ' +  returnVal.boundsChanged.toString() + ', sizeChanged: ' + returnVal.sizeChanged.toString() + ', needsNewImg: ' + returnVal.needsNewImg.toString() + '))';
	return returnVal;

	/*}*/
	/*else{
		console.log('***ERRERRERR - already comparing - ' + this._id);
		returnVal.msg = 'SKIP - comparing already';
		return returnVal;
	}*/
};
  ...
  /**
   * Refresh the map image (function called on an interval).
   */
  MapOverlay.prototype.refreshOnInterval = function(){
		var fCalledFrom = '[Refresh Interval ' + this._id.toString() + ']';
		//console.log(fCalledFrom);
		this.refresh(this._currentWidth, this._currentHeight, this._currentBounds, this._currentBoundsSW, this._currentBoundsNE, this._currentBoundsSW_lng, this._currentBoundsNE_lng, false, false, true, '[Refresh Interval]');
  }
  /**
   * Refresh the map image in current view port.
   */
  //MapOverlay.prototype.refresh = function (fMap, fBounds, fWidth, fHeight) {
//MapOverlay.prototype.refresh = function (fMap, fBounds, fWidth, fHeight) {}
MapOverlay.prototype.refresh = function (fWidth, fHeight, fBounds, fBoundsSW, fBoundsNE, fBoundsSW_lng, fBoundsNE_lng, fBoundsChanged, fSizeChanged, fNeedsNewImg, fCalledFrom) {
    var fCountRefresh = ++this._refreshCount; /* both vars will be the same, definitely increments this._refreshCount before setting - CLarson 8/13/18 */

  /* [QUESTION] Should this._last vars be declared here? Maybe not incase _checking is set to false before refresh has gotten to here yet ??? - CLarson 8/13/18 */

  /* Don't think this needs to be checked, I changed it to be set in .draw - CLarson 8/13/18
  if (this.drawing_ === true) {
    console.log("RETURN??? MapOverlay.refresh drawing_ -> needsNewRefresh_, return");  
  ...
  */
  ...
  var fParams = this.params;
    fParams.width = fWidth;
    fParams.height = fHeight;
    fParams.bounds = fBounds;
    ...
    triggerEvent_(this, 'drawstart'); /* Not positive about triggerEvent, I think it's needed for Jason's added part doOnDrawStart - CLarson 8/13/18 */
    if (this.__doOnDrawStart){
      this.__doOnDrawStart();
    }

    var self = this;
    var _name = this.mapService_.name

  /*----- */
    /* Set in draw() instead - CLarson 8/13/18 
    //this._drawing = true; */

  /* [QUESTION] Wait to remove overlay until arcgis has returned an image? - CLarson 8/13/18
  if (this.overlay_){
    /* Remove overlay_ => call ImageOverlay.onRemove - CLarson 8/~/18 *!!!
    console.log(self._id + " MO.refresh has overlay - " + this._lasthref); 
    this.overlay_.setMap(null);
    this.overlay_ = null;
    }*/

  this.mapService_.exportMap(fParams, function (json) {
      /* [QUESTION] How early should _drawing be set to false? Should it do anything? - CLarson 8/13/18 */
		//self._drawing = false;

		// Check against current once response is received
		var returnVal_current = self.checkBounds(fWidth, fHeight, fBoundsSW_lng, fBoundsNE_lng, 'current', false);
		if(!returnVal_current.compared){
			//console.log('  //' + fCalledFrom + ' export check current: no compare - ' + returnVal_current.msg);
		}
		else{
			//console.log('  //' + fCalledFrom + ' export check current - ' + returnVal_current.msg);
		}

		/* If exportMap returns a URL - CLarson 8/13/18 */
		if (json.href){
			/* If b/w/h are the same - CLarson 8/13/18 */
			if( !(returnVal_current.boundsChanged) || !(returnVal_current.sizeChanged) ){
				/* [LOG] - CLarson 8/13/18 */
				//console.log("+ REFRESH Draw: same bounds/resize " + self._id + ' ' + fBoundsSW_lng + ', ' + fBoundsNE_lng + fCalledFrom);
				/* Remove layer if one has been added - CLarson 8/13/18 */
				if (self.overlay_) {
					/* [LOG] - CLarson 8/13/18 */
					//console.log("*** REMOVE LAYER " + me._id + " ***"); 
					self.overlay_.setMap(null);		/* Remove overlay_ => call ImageOverlay.onRemove - CLarson 8/~/18 */
					self.overlay_ = null;
				}
				/* Create new layer, new params - CLarson 8/13/18 */
				/* modified to match jkirkland's version - clarson 8/17/18 */
				//Handle tokens    --added by jkirkland 6/12/2017
				var _href2 = json.href;
				if (_href2.indexOf("?") == -1 && _href2.indexOf("token=")==-1){
					_href2 += "?token=" + self.mapService_.__TOKEN;
				}
				self.overlay_ = new ImageOverlay(fSizeChanged, fWidth, fHeight, fBounds, fBoundsSW, fBoundsNE, _href2, self.map_, self.opacity_,self.__ZINDEX, self._id, fCalledFrom);		// previously: //self.overlay_ = new ImageOverlay(fSizeChanged, fWidth, fHeight, fBounds, fBoundsSW, fBoundsNE, json.href, self.map_, self.opacity_, self.__ZINDEX, self._id, fCalledFrom); // originally: me.overlay_ = new ImageOverlay(json.bounds, json.href, me.map_, me.opacity_,me.__ZINDEX, me._id);

				/* [QUESTION] Does _loadNew need to be set??? - CLarson 8/13/18 */
			}

			/* Else, current layer is different!!! - CLarson 8/13/18
			----- */
			else{
				/* [LOG] - CLarson 8/13/18 */
				//console.log("!!! REFRESH: different bounds, width, or height " + self._id + ' ' + fBoundsSW_lng + ', ' + fBoundsNE_lng + fCalledFrom);

				/*[CHECK HERE ASAP]******** MAYBE REMOVE LAYER */
			}
		}
		/* Else exportMap didn't return a URL - CLarson 8/13/18 */
		else{
			/* NOT SURE IF THIS PART IS NEEDED - CLarson 8/13/18 
			If nothing was returned because img is the same
			----- *!!!!!
			if(json.same){
			if((sameBounds && sameWidth && sameHeight)){
			  /* [LOG] - CLarson 8/13/18 *!!!
			  console.log("MapExport Received Same Params " + self._id + " (draw: " + fCountDraw + ' refresh: ' + fCountRefresh + ') [MO.refresh]');
			}
			}
			else{
			/* Layer is not drawn at that layer - CLarson 8/13/18 *!!!
			...
			}
			----- */
			...
			/* Remove layer if one has been added - CLarson 8/13/18 */
			if (self.overlay_) {
				//console.log('No Return => Remove Layer ' + self._id + ' ' + fBoundsSW_lng + ', ' + fBoundsNE_lng + fCalledFrom); 
				/* V3.32 CHRISTINA NOTE: Remove overlay_ - call ImageOverlay.onRemove */
				self.overlay_.setMap(null); /* Remove overlay_ => call ImageOverlay.onRemove - CLarson 8/~/18 */
				self.overlay_ = null;
			}
			//me.refresh(fMap, me._currentBounds, me._currentWidth, me._currentHeight);
		}
		
		// Check against current once response is received
		returnVal_current = self.checkBounds(fWidth, fHeight, fBoundsSW_lng, fBoundsNE_lng, 'current', false);
		if(!returnVal_current.compared){
			//console.log('  //' + fCalledFrom + ' END export check current: no compare - ' + returnVal_current.msg);
		}
		else{
			//console.log('  //' + fCalledFrom + ' END export check current - ' + returnVal_current.msg);
		}

		if (returnVal_current.boundsChanged || returnVal_current.sizeChanged ){
		  //console.log("NOW DIFFERENT - HOPEFULLY REFRESH IS CALLED ON IT'S OWN DRAW")
		  fCalledFrom = fCalledFrom + '[[REFRESH AGAIN]]';
		  //self.checkDraw(self._currentWidth, self._currentHeight, self._currentBounds, self._currentBoundsSW, self._currentBoundsNE, self._currentBoundsSW_lng, self._currentBoundsNE_lng, fCalledFrom);
		  return;
		}
		
		self._drawing = false;
		triggerEvent_(self, 'drawend');
		if (self.__doOnDrawEnd){
		  self.__doOnDrawEnd();
		}
...
/*		V3.32 CHRISTINA ADDED CONSOLE */
//		console.log("MapOverlay.show ");

    this.visible_ = true;
    this.div_.style.visibility = 'visible';
/*		V3.32 CHRISTINA - COMMENTED OUT REFRESH TO STOP LOOPING */
/*		NOT SURE WHAT THIS STOPS? */
    //this.refresh();
  };
  /**
   * Hide the overlay
...
console.log("MapOverlay.hide");
   ...
   * @param {Object} map
   */
function ImageOverlay(resize, width, height, bounds, sw, ne, url, map, op, __ZINDEX, id, mCalledFrom) {
  ...
  this.ne_ = ne;
  this.url_ = url;
  this.mCalledFrom_ = mCalledFrom;
  this.map_ = map;
  this.op_ = op;
  this.zIndex_ = __ZINDEX;
  this.id_ = id;

  this.div_ = null;
  //this.img_ = null; //UNNEEDED

  this.overlayLayer_ = null;
  ...
  /* V3.32 CHRISTINA NOTE - Calls ImageOverlay.onAdd */
  this.setMap(map);
}
  
  ImageOverlay.prototype = new G.OverlayView();
  ImageOverlay.prototype.onAdd0 = function() {
...
/* V3.32 CHRISTINA ADDED CONSOLE */
    //console.log(this.id_ + " IO.onAdd");
//loadOnce = false;

/* V3.32 CHRISTINA ADDING NON GLOBAL LOAD VARIABLES */
//this._imgAdded = true;

/* V3.32 CHRISTINA CHANGED TO LOWERCASE div */
    ...
    /* Don't set left or top, just width and height - CLarson 8/13/18 */
    div.style.width = this.width_ + 'px';
    div.style.height = this.height_ + 'px';
    div.style.zIndex = this.zIndex_;

	/* I don't know why an img element was created previously, but jkirklands new version doesn't do it - clarson 8/17/18 */
...
// Create the img element and attach it to the div.
    ...
    img.style.position = 'absolute';
    img.setAttribute('draggable', false); //NOT SURE IF DOES ANYTHING
	...
	*/
    ...
    this.overlayLayer_ = panes.overlayLayer; /* [REMOVE?] Redundant - CLarson 8/13/18 */
    panes.overlayLayer.appendChild(div);

    /* Set variable that ImageOverlay has been added - CLarson 8/13/18 */
    this._imgAdded = true;
	
  };

/* V3.32 CHRISTINA ADDED LIKE MapOverlay */
//ImageOverlay.prototype['onAdd'] = ImageOverlay.prototype.onAdd;
  ...
  ImageOverlay.prototype.draw = function() {
		...
		var swPrj = prj.fromLatLngToDivPixel(this.sw_);
		var nePrj = prj.fromLatLngToDivPixel(this.ne_);

		// Resize the image's DIV to fit the indicated dimensions.
		var div = this.div_;
		
		if(!(this._loaded) && !(this._removing)){
			/* [LOG] CLarson 8/~/18 */
			//console.log('    + ' + this.id_ + " IO.draw " + this.mCalledFrom_ + ' - LOADED - '  + this._imgAdded );
			div.style.left = swPrj.x + 'px';
			div.style.top = nePrj.y + 'px';
			/* These shouldn't be set ever, img will be removed and readded if resize occurs - CLarson 8/13/18 */
			//div.style.width = this.width_ + 'px';
			//div.style.height = this.height_ + 'px';
		}
		else if (this._loaded && !this._removing){
			//console.log('    + ' + this.id_ + " IO.draw " + this.mCalledFrom_ + ' - loaded, not removing - '  + this._imgAdded );
			div.style.left = swPrj.x + 'px';
			div.style.top = nePrj.y + 'px';
			/* have to recalc width and height - clarson */
			//div.style.width = this.width_ + 'px';
			//div.style.height = this.height_ + 'px';
			div.style.width = (nePrj.x - swPrj.x) + 'px'
			div.style.height = (swPrj.y - nePrj.y) + 'px';
		}
		else{
			//console.log('    SKIP ' + this.id_ + " IO.draw " + this.mCalledFrom_ + ' - removing -'  + this._imgAdded);
		}
...
/* V3.32 CHRISTINA ADDED CONSOLE */
	this._removing = true;
//console.log(this._id + ' IO.onRemove - added:' + this._imgAdded + ", loaded: " + this._loaded);
/** V3.32 CHRISTINA : ADDED IF **/
    if(this.div_){
//      console.log(this._id + " REMOVE CHILD");
      this.div_.parentNode.removeChild(this.div_);
    }
    else{
      //console.log(this._id + " NO DIV");
    }
