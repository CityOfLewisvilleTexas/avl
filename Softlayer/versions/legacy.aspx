...
<head>
	...
	<script src="components/map-service-obj-dark.js"></script>
	<script src="components/tile-cache-obj-dark.js"></script>
...
<div id="app" class="container-fluid" :class="[{night: nightMode, addClose: showCloseAll}]">
	...
	<alert :show="stationAlert > 0" placement="top" type="info" width="600px" duration="5000" dismissable>
		<p v-for="call in filteredCalls_Station">{{call.location}} {{'(' + call.callnumber + ')'}} {{'- ' + call.stationUnits}}</p>
	</alert>
				...
				<div id="leftSide" class="col-md-7 col-lg-8" v-bind:class="[{
					...
					<div id="mapDiv" v-show="displayToggle != 'List'">
						<map v-if="mapLoaded" :center.sync="centerV" :zoom.sync="zoomV" :bounds.sync="mapBounds" :options="mapOptions" v-on:g-idle="resizeMap" id="map" v-ref:map>
							...
							<marker v-for="vehicle in mappableVehicles" track-by="modemid" :position="getLatLng(vehicle.lat, vehicle.lng)" :clickable.sync="true" :draggable.sync="false" :icon="getVehicleIcon(vehicle)" :label="getVehicleLabel(vehicle)" @g-click="clickVehicleMarker(vehicle, $event)" :z-index="getVehicleIndex(vehicle)"> <!--:z-index="getVehicleIndex(vehicle)"-->
								<info-window class="iwStyle" :opened.sync="vehicle.isOpen" :z-index="getVehicleIndex(vehicle)">
									...
									<table class="table table-condensed">
										...
										<tr><td><small>Est Speed:</small></td><td><small>{{vehicle.velocity}}mph&nbsp;{{vehicle.compass}} <span v-html="getBearingArrow(vehicle.compass)"></span></small></td></tr>
									</table>
							...
							<map-service-obj-dark v-for="lyr in filterDarkLayersByFiletype('mapservice')" :layer.sync="lyr"></map-service-obj-dark>
							<tile-cache-obj-dark v-for="lyr in filterDarkLayersByFiletype('tilecache')" :layer.sync="lyr"></tile-cache-obj-dark>
						</map>
					</div>
...
var app = new Vue({
	...
	watch:{
		validOAUTH: function(newVal, oldVal){
			if(newVal != oldVal){
				if(!(newVal)){
	...
	computed: {
		...
		mapOptions: function(){
			if(!(this.nightMode)){
				return{fullscreenControl: false, streetViewControl: true, gestureHandling: 'greedy', styles: null};
			}
			else{
				return{fullscreenControl: false, streetViewControl:true, gestureHandling: 'greedy', styles: this.night};
			}
		},
	...
	methods:{
		...
		getBearingArrow: function(_bearing){
			var _arrow = '&larr;'; //'←'  '→'  '↓  ↖  ↗  ↘  ↙'
			if ( ['N'].indexOf(_bearing) > -1 ){_arrow = '&larr;'}
			if ( ['NNE','NE','ENE'].indexOf(_bearing) > -1 ){_arrow = '&#8599;'}
			if ( ['E'].indexOf(_bearing) > -1 ){_arrow = '&rarr;'}
			if ( ['ESE','SE','SSE'].indexOf(_bearing) > -1 ){_arrow = '&#8600;'}
			if ( ['S'].indexOf(_bearing) > -1 ){_arrow = '&darr;'}
			if ( ['SSW','SW','WSW'].indexOf(_bearing) > -1 ){_arrow = '&#8601;'}
			if ( ['W'].indexOf(_bearing) > -1 ){_arrow = '&larr;'}
		...
		pollingCalls: function(){
			...
			function(response){
				...
				if(!(response.data.results[0]["ErrorMessage"])){
					var unitArr;
					var u;
				...
				else if(response.data.results[0]["ErrorMessage"] == "Google did not recognize the oauth token"){
					app.callsPolling = false;
					app.validOAUTH = false;
				}
				else{
					app.callsPolling = false;
					if (app.callsPollingSec != 'OFF'){
	...
	ready: function() {
		console.log('AVL 3.4 Polling Version using legacy webservice updated 7/19/17');
